...
export default function ActivityLog() {
  ...
  const { data: payload } = useQuery({
    ...
    queryFn: async () => (await adminAPI.logs(filters)).data?.data ?? {},
  });
  ...
  return (
    <div className="bg-white border rounded-4 p-4 shadow-sm">
      <div className="d-flex flex-column flex-lg-row justify-content-between gap-3 mb-4">
        <div>
          <h2 className="h4 mb-1">Nhật ký hoạt động</h2>
          <p className="text-muted mb-0">Lọc theo người dùng, phân hệ và ngày để theo dõi lịch sử thao tác trong hệ thống.</p>
        </div>
